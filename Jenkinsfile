pipeline {
    ...
    parameters {
        choice(
            name: 'ENVIRONMENT',
            choices: ['develop', 'qa', 'release.s.2025.08', 'release.s.2025.09', 'release.s.2025.10', 'main'],
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: "${BRANCH}",
                    url: 'https://github.com/JuanPerez012/teacher-service.git',
        ...
        stage('Copy .env') {
            steps {
                sh """
                    mkdir -p ${DEPLOY_DIR}
                    cp /apps/config/teacher/.env ${DEPLOY_DIR}/${ENV_DEPLOY_FILE}
